...
st.caption(
    "CSV 파일을 업로드하면 주소별 주차요금을 검색하고 지도에서 확인할 수 있습니다."
)
...
def read_csv_safely(uploaded_file):
    ...
    raise ValueError(
        "CSV 파일을 읽지 못했습니다. CSV 형식과 문자 인코딩을 확인해 주세요."
    )
...
if uploaded_file is None:
    st.info("왼쪽에서 공영주차장 CSV 파일을 업로드해 주세요.")
...
parking_df = pd.DataFrame(index=original_df.index)
...
if latitude_column and longitude_column:
    ...
    parking_df["latitude"] = float("nan")
    parking_df["longitude"] = float("nan")
...
if map_df.empty:
    st.warning(
        "지도에 표시할 위도와 경도 데이터가 없습니다. "
        "CSV에 위도·경도 열을 추가하거나 열 연결 설정을 확인해 주세요."
    )
    st.stop()


map_df["tooltip_name"] = map_df["parking_name"].apply(
    lambda value: html.escape(str(value))
)

map_df["tooltip_address"] = map_df["address"].apply(
    lambda value: html.escape(str(value))
)

map_df["tooltip_fee"] = map_df["fee"].apply(
    lambda value: html.escape(str(value))
)

center_latitude = float(map_df["latitude"].mean())
center_longitude = float(map_df["longitude"].mean())
