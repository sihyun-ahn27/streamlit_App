...
def weighted_knn_recommend(
    ...
    """표준화와 가중 유클리드 거리를 직접 계산하는 KNN.

    scikit-learn 없이 Pandas와 NumPy만 사용한다.
    각 변수는 z=(x-평균)/표준편차로 표준화한 뒤,
    사용자가 정한 가중치를 반영하여 거리를 계산한다.
    """
    if reference_name not in set(df["상품명"]):
        raise ValueError("기준 상품을 찾을 수 없습니다.")
    ...
    if not features:
        raise ValueError("선택한 변수가 데이터에 없습니다.")

    candidates = candidates.dropna(subset=features).copy().reset_index(drop=True)

    if reference_name not in set(candidates["상품명"]):
        missing = [f for f in features if pd.isna(reference[f])]
        raise ValueError(
            "기준 상품에 선택 변수의 결측값이 있습니다: " + ", ".join(missing)
        )

    if len(candidates) < 2:
        raise ValueError(
            "추천할 비교 상품이 부족합니다. 카테고리 제한을 해제하거나 데이터를 늘려 주세요."
        )

    X = candidates[features].astype(float).to_numpy()
    means = np.nanmean(X, axis=0)
    stds = np.nanstd(X, axis=0, ddof=0)

    # 값이 모두 같은 변수는 표준편차가 0이므로 1로 바꿔 거리 계산에서 영향이 없게 한다.
    safe_stds = np.where(np.isclose(stds, 0), 1.0, stds)
    X_standard = (X - means) / safe_stds

    weight_array = np.array([weights[f] for f in features], dtype=float)
    sqrt_weights = np.sqrt(weight_array)
    X_weighted = X_standard * sqrt_weights

    ref_pos = int(
        np.flatnonzero(candidates["상품명"].to_numpy() == reference_name)[0]
    )
    ref_vector = X_weighted[ref_pos]

    distances = np.sqrt(np.sum((X_weighted - ref_vector) ** 2, axis=1))
    ...
    if not recommendations.empty:
        ...
        recommendations["나트륨차이"] = (
            recommendations["나트륨"] - reference["나트륨"]
        )
        recommendations["단백질차이"] = (
            recommendations["단백질"] - reference["단백질"]
        )
...
class SimpleOLSResult:
    ...
    def conf_int(self) -> pd.DataFrame:
        return pd.DataFrame({0: self.lower95, 1: self.upper95})


def _normal_two_sided_pvalue(t_value: float) -> float:
    """정규근사를 이용한 양측 p값. scipy 없이 계산한다."""
    return float(math.erfc(abs(float(t_value)) / math.sqrt(2)))
...
def run_hedonic_regression(
    ...
    """NumPy로 구현한 탐색적 헤도닉 가격 회귀.

    가격을 열량·내용량·영양성분·카테고리로 설명한다.
    HC3 강건 공분산을 사용하되, p값은 큰 표본 정규근사로 계산한다.
    """
    data = df.copy()
    ...
    numeric_features = ["calories", "amount", "sugar", "sodium", "protein"]
    X_df = reg[numeric_features].astype(float).copy()
    ...
    y = reg["price"].to_numpy(dtype=float)
    n, p = X.shape
    ...
    denom = np.clip(1 - hat_diag, 1e-8, None)
    hc3_scale = (residual / denom) ** 2
    meat = X.T @ (X * hc3_scale[:, None])
    covariance = xtx_inv @ meat @ xtx_inv

    standard_errors = np.sqrt(
        np.clip(np.diag(covariance), a_min=0, a_max=None)
    )
    t_values = np.divide(
        beta,
        standard_errors,
    ...
    p_values = np.array(
        [_normal_two_sided_pvalue(value) for value in t_values],
        dtype=float,
    )

    index = X_df.columns.astype(str)
    params = pd.Series(beta, index=index)
    bse = pd.Series(standard_errors, index=index)
    pvalues = pd.Series(p_values, index=index)
    lower95 = params - 1.96 * bse
    upper95 = params + 1.96 * bse

    model = SimpleOLSResult(
        nobs=int(n),
        rsquared=float(r_squared),
        rsquared_adj=float(r_squared_adj),
        params=params,
        bse=bse,
        pvalues=pvalues,
        lower95=lower95,
    ...
    reg["predicted_price"] = fitted
    reg["residual"] = residual
    reg["abs_residual"] = np.abs(residual)
    return model, reg


def coefficient_table(model: SimpleOLSResult) -> pd.DataFrame:
    conf = model.conf_int()
    return pd.DataFrame(
        {
...
def load_sample_data() -> pd.DataFrame:
    """sample_data.csv를 읽고, 파일이 없으면 내장 샘플을 사용한다."""
...
if page == "홈":
    ...
    with tab2:
        ...
        try:
            ...
            st.markdown("#### 회귀계수")
            st.caption("강건 표준오차는 HC3 방식으로 계산했으며, p값은 외부 통계 패키지 없이 큰 표본 정규근사를 사용합니다.")
